...
def fetch_book_from_api(bookTitle):
    url = f"https://www.googleapis.com/books/v1/volumes?q=intitle:{bookTitle}&langRestrict=pt&printType=books&maxResults=1"
...
def recommenderBooks_multi(bookTitles, nRecommendation=5, weight_structured=0.6, weight_text=0.4):
    all_sim_struct = []
    all_sim_text = []

    for bookTitle in bookTitles:
        idx = dfBooks.index[dfBooks['title'].str.contains(bookTitle, case=False)]

        if len(idx) == 0:
            book = fetch_book_from_api(bookTitle)
            if book is None:
                continue
            ...
            book_row = dfBooks.iloc[idx[0]]
            book = {
                'title': book_row['title'],
                'categories': [cat.strip() for cat in book_row['categories'].split(',')],
                'authors': [auth.strip() for auth in book_row['authors'].split(',')],
                'pageCount': book_row['pageCount'],
                'averageRating': book_row['averageRating'],
                'desc_clean': book_row['desc_clean']
            }
            cosine_sim_text = linear_kernel(tfidf_matrix[idx[0]], tfidf_matrix).flatten()
    ...
    mask = ~dfBooks['title'].str.contains('|'.join(bookTitles), case=False, na=False)
    ...
    return recs[['id', 'title', 'authors', 'categories', 'pageCount', 'averageRating', 'Similarity']].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    recs = recommenderBooks_multi(titulos, nRecommendation=5)
